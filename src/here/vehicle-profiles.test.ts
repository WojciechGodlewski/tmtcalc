...
describe('VehicleProfiles', () => {
  describe('VEHICLE_PROFILES', () => {
    it('defines van_8ep profile', () => {
      const profile = VEHICLE_PROFILES.van_8ep;
      expect(profile.grossWeight).toBe(3500);
      expect(profile.height).toBe(2.7);
      expect(profile.width).toBe(2.2);
      expect(profile.length).toBe(6.5);
      expect(profile.axleCount).toBe(2);
    });

    it('defines solo_18t_23ep profile', () => {
      const profile = VEHICLE_PROFILES.solo_18t_23ep;
      expect(profile.grossWeight).toBe(18000);
      expect(profile.height).toBe(3.6);
      expect(profile.width).toBe(2.55);
      expect(profile.length).toBe(10.0);
      expect(profile.axleCount).toBe(2);
    });

    it('defines ftl_13_6_33ep profile', () => {
      const profile = VEHICLE_PROFILES.ftl_13_6_33ep;
      expect(profile.grossWeight).toBe(40000);
      expect(profile.height).toBe(4.0);
      expect(profile.width).toBe(2.55);
      expect(profile.length).toBe(16.5);
  ...
  describe('profile constraints', () => {
    ...
    it('all profiles have valid dimensions', () => {
